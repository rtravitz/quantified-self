...
function addExerciseRow(name, calories) { $('.exercise-table tr:first').after(
    '<tr id="' + name + '-row"><td class="name-cell">' + name + 
    '</td><td class="calories-cell">' + calories + '</td><td><a href="#" id="' +
...
function addFood() {
  ...
  if (food.name === "") {
    ...
    $('input[name="name"]')
      .after('<p class="calories-err">Please enter a calorie amount.</p>')
  } else {
    food.store();
...
function addExercise() {
  ...
  if (exercise.name === "") {
    $('input[name="name"]')
      .after('<p class="name-err">Please enter an exercise name.</p>')
